namespace PuzzleCMS.UnitsTests.Multitenancy
{
    ...
    {
        private const string RequestPathTenant1_1 = "/tenant-1-1";
        private const string RequestPathTenant1_2 = "/tenant-1-2";
        private const string RequestPathTenant1_3 = "/tenant-1-3";
        ...
        {
            TestHarness harness = new TestHarness();
            HttpContext context = CreateContext(RequestPathTenant1_1);

            TenantContext<AppTenant> tenantContext = await harness.CachingAppTenantResolver.ResolveAsync(context);

            Assert.True(harness.Cache.TryGetValue(RequestPathTenant1_2, out TenantContext<AppTenant>  cachedTenant));
        ...
        public async Task CanRetrieve_TenantContext_FromCache()
        {
            ...
            Assert.True(harness.Cache.TryGetValue(RequestPathTenant1_2, out TenantContext<TestTenant> cachedTenant));
        ...
        public async Task ShouldDisposeTenantOnEviction_FromCacheByDefault()
        {
            ...
            harness.Cache.Get(RequestPathTenant1_2);

            // and give it a moment to works its magic.
            Thread.Sleep(100);

            // should also expire tenant context by default
            Assert.False(harness.Cache.TryGetValue(RequestPathTenant1_1, out TenantContext<TestTenant> cachedTenant), "Tenant Exists");
        ...
        public async Task ShouldEvictAllCacheEntriesOf_TenantContextByDefault()
        {
            ...
            // first request
            TenantContext<TestTenant> tenantContext = await harness.Resolver.ResolveAsync(CreateContext(RequestPathTenant1_1));

            // cache should have all 3 entries
            Assert.NotNull(harness.Cache.Get(RequestPathTenant1_1));
            Assert.NotNull(harness.Cache.Get(RequestPathTenant1_2));
            Assert.NotNull(harness.Cache.Get(RequestPathTenant1_3));

            // expire
            harness.Cache.Remove(RequestPathTenant1_1);

            Thread.Sleep(500);

            // look it up again so it registers
            harness.Cache.TryGetValue(RequestPathTenant1_1, out TenantContext<TestTenant>  cachedTenant);

            Thread.Sleep(500);

            // pear is expired - because apple is
            Assert.False(harness.Cache.TryGetValue(RequestPathTenant1_3, out cachedTenant), "Tenant Exists");
        ...
        public async Task CanEvictSingleCacheEntry_OfTenantContext()
        {
            ...
            await harness.Resolver.ResolveAsync(CreateContext(RequestPathTenant1_1));

            // wait 1 second
            Thread.Sleep(1000);

            // second request
            await harness.Resolver.ResolveAsync(CreateContext(RequestPathTenant1_2));
            ...
            // pear is not expired
            Assert.True(harness.Cache.TryGetValue(RequestPathTenant1_2, out cachedTenant), "Tenant Does Not Exist");
        }
